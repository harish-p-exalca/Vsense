...
export class DeviceassignparamComponent implements OnInit {
  ...
  displayedColumns: string[] = ['pramID', 'title', 'unit','longText','min','max','activityGraphTitle','status','action'];
  row:FormGroup;
...
CreateAppUsage(): void {
      ...
      for(var i in this.deviceparams){ 
        if(this.deviceparams[i].paramID==param){
          control.patchValue({
            ...
            activityGraphTitle:this.deviceparams[i].title
          });
  ...
  actionselect(value:string,index){
    let data=this.mform.value;
    var pramid=data.parameters[index].pramID;
    let pstatus:boolean;
    for(var i in this.deviceparams){
      if(this.deviceparams[i].paramID==pramid){
        pstatus=this.deviceparams[i].isEnabled;
        break;
      }
    }
    if(value=='1'){
      if(pstatus){
        this.notification.update("parameter already enabled");
      }
      else{
        this.service.enabledeviceparam(this.deviceID,pramid).subscribe(x => {
          this.deviceparams[i].isEnabled=true;
          this.notification.success("parameter enabled");
        },(err)=>{
          this.notification.success("something went wrong");
        });
      }
      ...
      if(!pstatus){
        this.notification.update("parameter already disabled");
      }
      else{
        this.service.disabledeviceparam(this.deviceID,pramid).subscribe(x => {
          this.deviceparams[i].isEnabled=false;
          this.notification.success("parameter disbled");
        },(err)=>{
          this.notification.success("something went wrong");
        });
      }
